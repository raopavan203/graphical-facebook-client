    ...
    this.generateTheater = function(container) {
        function cameraRoll(){
            var roll = container.append("g")
                        .attr("id","cameraRoll");
            var d;
            for(var i=0; i < that.theater.data ; i++){     
		d=that.theater.data[i];
           	     
            	roll.append("clipPath")
                        .attr("id",function(){ return "picClip" + d.id; })
                            .append("rect")
                                .attr("x", function() { return i*110 + 25; })
                                .attr("y" , h-85)
                                .attr("width", 100)
                                .attr("height" , 75);
            	...
            	roll.append("image")
                        .attr("height", 75)
                        .attr("width", function() { return d.width/d.height * 75; })
                        .attr("x", function() { return ((i * 110) + 20) - ((d.width/d.height * 75) - 100)/2; })
                        .attr("y", h - 85)
                        .attr("xlink:href", function() { return d.thumbnail; })
                        .attr("clip-path", function() { return "url(#picClip" + d.id + ")";});
	    }
        }
        
        if(that.theater.data === undefined)
            $.getJSON($SCRIPT_ROOT + '/_getData', { a: accessToken, b: me.id, c:"photos" }, function(response){
                var arrayLength = response.photos.data.length;
                var pic = new Object();
                that.theater.data = new Array();
