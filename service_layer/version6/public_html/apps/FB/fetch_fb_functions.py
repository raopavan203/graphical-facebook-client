...
def getFriendLists(accessToken, all_data):
     
    """returns persons in user's friend lists of type 1=close_friends 2=work 3=education 4=user_created 5=current_city"""
    print '\n\n%%%%%%%%%%%%%%%%  FRIEND LIST'
    if 'friendlists' in all_data.keys():
       #print all_data['friendlists']
       if 'data' in all_data['friendlists'].keys():
           #print all_data['friendlists']['data']
           friendlist_data={}
           ...
           for list_elem in all_data['friendlists']['data']:
               if 'list_type' in list_elem.keys():
                 if 'members' in list_elem.keys():
...
def splitPhotos(accessToken, all_data, uid, pics_by_me, pics_by_others):
    print 'in splitPhotos'
...
def splitPhotosByTagCount(pics, few_tags_pics, many_tags_pics):
...
def getPhotos(accessToken, pics, pics_with_tags, uid):
...
def getCheckins(accessToken, uid, all_data):
...
def splitFeed(accessToken, all_data, uid, feed_by_me, feed_by_others):
    print 'in splitFeed'
...
def getLinks(accessToken, all_data):
...
def getFamily(accessToken, all_data):
...
def getInbox(accessToken, uid, all_data):
...
def get_status(accessToken, uid, all_data):
