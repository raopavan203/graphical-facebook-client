...
function initializeData(uid,accessToken,callback){
    ...
    $.getJSON($SCRIPT_ROOT + '/_getProfile', { a: accessToken }, function(data) {
        d3.select("#logout")
            .on("click",function(){ FB.logout(function(res){  window.location.reload();  });})
            .html("logout");
...
var popUpLogin = function(){
    ...
    FB.login(function(response) {
       if (response.authResponse) {
         ...
         loggedIn(response.authResponse.userID,response.authResponse.accessToken,init);                        
...
var loginLand = function(){
        ...
        thisSprite.append("image")
            ...
            .attr("x",-25)
            .attr("y",0)
            .attr("xlink:href","static/images/facebook-default.jpg")
            .attr("width",picWidth)
            .attr("height",picHeight)
	        .attr("preserveAspectRatio","none")	
...
var loggedIn = function(userID,accessToken,callback){
    d3.select(".spriteOuterCircle.sprite_login")
        .transition()
            .duration(2000)
            .attr("r",w)
            .style("fill","#202020")
            .each("end",function(){
                ...
                initializeData(userID,accessToken,callback);
            });
}
